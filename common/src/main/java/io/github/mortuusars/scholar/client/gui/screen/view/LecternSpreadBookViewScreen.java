    ...
    protected void createBottomButtons() {
        if (player.mayBuild()) {
            if (Config.Client.SHOW_DONE_BUTTON.get()) {
                this.addRenderableWidget(new Button(this.width / 2 - 100, topPos + BOOK_HEIGHT + 12, 98, 20,
                        CommonComponents.GUI_DONE,
                        button -> this.onClose()));

                this.addRenderableWidget(new Button(this.width / 2 + 2, topPos + BOOK_HEIGHT + 12, 98, 20,
                        Component.translatable("lectern.take_book"),
                        button -> this.sendButtonClick(3)));
            } else {
                this.addRenderableWidget(new Button(this.width / 2 - 60, topPos + BOOK_HEIGHT + 12, 120, 20,
                        Component.translatable("lectern.take_book"),
                        (button) -> this.sendButtonClick(3)));
            }
        } else {
